...
v_r_neur[:, 0] = -v_r_neur[:, 0]#flip the first eigenvectors to be positive
...
l_r = np.load(eig_tuning_dir + 'lin_rf_resp_' + fn + '.npy',)
...
for k in tqdm(range(nrows)):
    ...
    ms_lr_beta = ms_lr*beta
    resp.append([ms_lr_beta, 
                np.argsort(ms_lr_beta)[::-1], 
                np.argsort(v_r_stim[:,k])[::-1]],)
                #np.argsort(ms_lr_beta-v_r_stim[:,k])[::-1]],)
sc = 1.5
...
for row in range(nrows):
    ...
    coef_axs[row][0].plot(resp[row][0][:n_ims], c=lin_color, label='Linear component')
...
[ax.imshow(rf[i][:,90:-90], cmap='gray') for i, ax in enumerate(rf_axs)]
...
for row, gs in enumerate(img_gs):
    ...
    for q in [0, 1, 2, -3, -2, -1]:
        ...
        img_axs[row][1][q].imshow(imgs[:, 90:-90, resp[row][1][q]], cmap='gray')
        img_axs[row][0][q].imshow(imgs[:, 90:-90, resp[row][2][q]], cmap='gray')
        for spine in img_axs[row][1][q].spines.values():
            spine.set_edgecolor(lin_color)
            spine.set_linewidth(1)
plt.savefig('PCA_example'+fn+'.png', 
...
plt.savefig('./linear_rfs_all.png', bbox_inches='tight', transparent=True, dpi=1000)
